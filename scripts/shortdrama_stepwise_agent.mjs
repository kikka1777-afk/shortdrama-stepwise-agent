...
把用户素材重构成可拍摄短剧故事正文。中文输出，700-1100 字。强化开场冲突、人物欲望、反转、爽点和结尾钩子。不要分集，不要解释。`,
...
根据剧本和分集剧本提炼主要角色，最多 8 人。中文字段，image_prompt 用英文，并以 "${styleFrag(style)} --ar 9:16" 结尾。严格返回 JSON，不要代码块，不要解释。
JSON 格式：{"characters":[{"name":"","role":"","desire":"","conflict":"","appearance":"","voice":"","image_prompt":""}]}`,
    assets: `# Role: 美术资产 Agent
根据剧本、分集和角色设定，先提取真正需要统一设计或单独出图的视觉资产。包含角色、场景、道具、封面/标题卡等。每项 image_prompt 用英文，并以 "${styleFrag(style)} --ar 9:16" 结尾。严格返回 JSON，不要代码块，不要解释。
JSON 格式：{"groups":[{"category":"角色造型","items":[{"name":"","tag":"","note":"","appears_in_episodes":[1],"image_prompt":""}]}]}`,
    storyboard: `# Role: 分镜剧本 Agent
把分集剧本拆成可拍摄关键分镜，并严格参考素材清单保持角色、场景、道具和视觉风格一致。每集输出 3-5 个镜头。中文 visual，英文 visual_prompt，并以 "${styleFrag(style)} --ar 9:16" 结尾。严格返回 JSON，不要代码块，不要解释。
...
function printHelp() {
  console.log(`Usage:
  list
  prompt <step> [--input file] [--tags 都市,逆袭] [--total 4] [--style "Studio Realistic"]
